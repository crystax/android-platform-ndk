...
for PLATFORM in $PLATFORMS; do
    for ABI in $(abis_for_platform $PLATFORM); do
        echo ""
        echo "================================================================="
        echo "===> Testing CrystaX headers for $PLATFORM/$ABI <==="
        echo "================================================================="
        echo ""
        ...
        run $NDK/ndk-build -C $MYDIR -B APP_PLATFORM=$PLATFORM APP_ABI=$ABI V=1
        if [ $? -ne 0 ]; then
            echo "ERROR: Can't compile CrystaX headers for $PLATFORM/$ABI alone" 1>&2
            exit 1
        fi
        echo ""
        echo "===> OK: CrystaX headers for $PLATFORM/$ABI"
    done
done
